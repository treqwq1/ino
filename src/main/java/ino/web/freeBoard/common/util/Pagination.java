...
public class Pagination {
	...
	public String getPagination() {
		...
		if(this.getBlockStart() > 0) {
			buffer.append("<span><a href=\'javascript:caseselect(" + (this.page-1) + ")'\"><</a></span>");
		}
		
		for(int i = this.getBlockStart(); i <= this.getBlockEnd(); i++) {
			if(page == i )
				buffer.append("<span><a href=\'javascript:caseselect(" + i + ")'\">" + i + "</a></span>");
			else
				buffer.append("<span><a href=\'javascript:caseselect(" + i + ")'\">" + i + "</a></span>");
		}
		
		if(this.getBlockEnd() < endPage) {
			buffer.append("<span><a href=\'javascript:caseselect(" + this.getBlockNext() + ")'\">></a></span>");
		}
		
		buffer.append("</div>");
		System.out.println("blockprev" + this.getBlockPrev() );
